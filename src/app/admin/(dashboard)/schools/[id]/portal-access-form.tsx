...
export default function PortalAccessForm({ schoolId }: { schoolId: number }) {
  ...
  return (
    <form action={formAction} className="border-t border-line p-3">
      <input type="hidden" name="school_id" value={schoolId} />
      <div className="grid grid-cols-4 items-end gap-2">
        <div>
          <label className="block text-xs text-ink-400">Email</label>
          <input name="email" type="email" required className={inputClass} />
        </div>
        <div>
          <label className="block text-xs text-ink-400">Name</label>
          <input name="name" className={inputClass} />
        </div>
        <div>
          <label className="block text-xs text-ink-400">Password (share with them)</label>
          <input name="password" type="text" required minLength={8} className={inputClass} />
        </div>
        <button
          type="submit"
          disabled={pending}
          className="h-[38px] rounded-md bg-teal-600 px-3 text-sm font-medium text-white hover:bg-teal-700 disabled:opacity-50"
        >
